...
class Control {
  ...
  drawSpokes(ctx) {
    ...
    for (var i = 0; i < 36; i++) {
      ctx.beginPath();
      ctx.moveTo(...this.pos);
      ctx.lineTo(...this.arcPos(Math.PI / 18 * i));
      ctx.stroke();
    }
  ...
  arcPos(angle, radius=0) {
    return [ this.pos[0] + Math.cos(angle) * (radius || this.radius),
             this.pos[1] + Math.sin(angle) * (radius || this.radius) ];
  }
